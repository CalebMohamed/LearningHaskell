...
-- not quite what i think its looking for
-- sumInts a b = mySum [a..b]
--     where mySum [] = 0
...
higherOrderSequenceApplication op f a b z
    | a > b     = z
    | otherwise = f (op a) (higherOrderSequenceApplication op f (a+1) b z)

-- Define a factorial method using the higherOrderSequenceAppliction
hoFactorial :: Int -> Int
hoFactorial a = higherOrderSequenceApplication id (*) 1 a 1
